...
COPY . .
RUN go build -mod=vendor -o tools -ldflags="-s -w" ./app/tools/cmd/tools/

FROM registry.cn-hangzhou.aliyuncs.com/pf94514203/yunchuang:base
COPY --from=builder /go/src/tools /
WORKDIR /
...
ENV PARAMS=""
##ENTRYPOINT
CMD ["sh","-c","mv /data/tools.log /data/tools.$(date '+%Y%m%d%H%M%S').log || true && /tools $PARAMS >/data/tools.log 2>&1"]