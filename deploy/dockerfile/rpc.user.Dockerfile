...
COPY . .
RUN go build -mod=vendor -o user -ldflags="-s -w" ./app/user/cmd/user/

FROM registry.cn-hangzhou.aliyuncs.com/pf94514203/yunchuang:base
COPY --from=builder /go/src/user /
WORKDIR /
...
ENV PARAMS=""
##ENTRYPOINT
CMD ["sh","-c","mv /data/user.log /data/user.$(date '+%Y%m%d%H%M%S').log || true && /user $PARAMS >/data/user.log 2>&1"]