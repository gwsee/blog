      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-8">
            <!-- 项目标题区域 -->
            <div class="bg-white/50 rounded-2xl p-8 shadow-lg">
              <h1 class="text-4xl font-bold text-[#333] mb-4">{{ project.title }}</h1>
              <div class="flex flex-wrap gap-4 items-center text-sm text-[#444]">
            ...
            <div class="space-y-4" v-if="project.skills && project.skills.length>0">
              <h2 class="text-2xl font-bold text-[#333] flex items-center space-x-2">
                <CodeBracketIcon class="w-6 h-6" />
                <span>Technologies & Tools</span>
              </h2>
              <div class="grid gap-4">
                <div class="bg-white/50 rounded-xl p-6 shadow-lg">
                  <div class="flex flex-wrap gap-3">
                    <span
                        v-for="skill in project.skills"
                        :key="skill"
                        class="px-4 py-2 bg-white/60 rounded-full text-sm text-[#444] hover:bg-white/70 transition-colors duration-300"
                    >
                      {{ skill }}
                    </span>
                  </div>
                    ...
                    <div class="absolute bottom-0 p-4 w-full">
                      ...
                      <div class="flex flex-wrap gap-2" v-if="false">
                        <span
                            ...
                            :key="tech"
                            class="px-2 py-1 bg-white/60 rounded text-xs text-[#444]"
                        >
                          {{ tech }}
                        </span>
                      </div>
          ...
          <div class="space-y-6">
            <!-- Key Achievements -->
            <div class="bg-white/50 rounded-2xl p-6 shadow-lg">
              <h2 class="text-2xl font-bold text-[#333] mb-6 flex items-center space-x-2">
                <TrophyIcon class="w-6 h-6" />
                <span>Key Achievements</span>
              </h2>
              <div class="space-y-6">
                <div class="space-y-2">
                  <h3 class="text-lg font-semibold text-[#333]">项目描述</h3>
                  <p class="text-[#444] leading-relaxed" v-html="formattedContent(project.description)"></p>
                </div>
                <div class="space-y-2">
                  <h3 class="text-lg font-semibold text-[#333]">项目周期</h3>
                  <div class="flex justify-between text-[#444]">
                    <span>开始时间: {{ $formatDate(project.start,'{y}-{m}') }}</span>
                    <span>结束时间: {{ $formatDate(project.end,'{y}-{m}') }}</span>
                  </div>
                </div>
                <div class="space-y-2">
                  <h3 class="text-lg font-semibold text-[#333]">项目状态</h3>
                  <div class="flex items-center space-x-2 text-[#444]">
                    <span
