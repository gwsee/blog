      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-8">
            <!-- 项目标题区域 -->
            <div class="bg-[#141414]/60 rounded-2xl p-8 shadow-lg">
              <h1 class="text-4xl font-bold text-[#F8F8F8] mb-4">{{ project.title }}</h1>
              <div class="flex flex-wrap gap-4 items-center text-sm text-[#EEDC82]">
                ...
                <a
                    ...
                    :href="project.link"
                    target="_blank"
                    class="flex items-center space-x-2 text-[#EEDC82] hover:text-blue-300"
                >
            ...
            <div class="space-y-4" v-if="project.skills && project.skills.length>0">
              <h2 class="text-2xl font-bold text-[#F8F8F8] flex items-center space-x-2">
                <CodeBracketIcon class="w-6 h-6" />
                <span>Technologies & Tools</span>
              </h2>
              <div class="grid gap-4">
                <div class="bg-[#141414]/60 rounded-xl p-6 shadow-lg">
                  <div class="flex flex-wrap gap-3">
                    <span
                        v-for="skill in project.skills"
                        :key="skill"
                        class="px-4 py-2 bg-[#0a0a0a]/70 rounded-full text-sm text-[#EEDC82] hover:bg-[#0a0a0a]/80 transition-colors duration-300"
                    >
                      {{ skill }}
                    </span>
                  </div>
                ...
                >
                  ...
                  <div class="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent">
                    <div class="absolute bottom-0 p-4 w-full">
                      <h3 class="text-lg font-semibold text-[#F8F8F8] mb-1" v-if="false">Project {{ index + 1 }}</h3>
                      <div class="flex flex-wrap gap-2" v-if="false">
                        <span
                            v-for="tech in project.skills"
                            :key="tech"
                            class="px-2 py-1 bg-[#141414]/60 rounded text-xs text-[#EEDC82]"
                        >
                          {{ tech }}
                        </span>
                      </div>
          ...
          <div class="space-y-6">
            <!-- Key Achievements -->
            <div class="bg-[#141414]/60 rounded-2xl p-6 shadow-lg">
              <h2 class="text-2xl font-bold text-[#F8F8F8] mb-6 flex items-center space-x-2">
                <TrophyIcon class="w-6 h-6" />
                <span>Key Achievements</span>
              </h2>
              <div class="space-y-6">
                <div class="space-y-2">
                  <h3 class="text-lg font-semibold text-[#F8F8F8]">项目描述</h3>
                  <p class="text-[#EEDC82] leading-relaxed" v-html="formattedContent(project.description)"></p>
                </div>
                <div class="space-y-2">
                  <h3 class="text-lg font-semibold text-[#F8F8F8]">项目周期</h3>
                  <div class="flex justify-between text-[#EEDC82]">
                    <span>开始时间: {{ $formatDate(project.start,'{y}-{m}') }}</span>
                    <span>结束时间: {{ $formatDate(project.end,'{y}-{m}') }}</span>
                  </div>
                </div>
                <div class="space-y-2">
                  <h3 class="text-lg font-semibold text-[#F8F8F8]">项目状态</h3>
                  <div class="flex items-center space-x-2 text-[#EEDC82]">
                    <span
