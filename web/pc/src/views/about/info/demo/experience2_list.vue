<template>
  <div class="min-h-screen text-white p-4 md:p-8" style="background: transparent;">
    <div class="max-w-6xl mx-auto overflow-hidden">
      <div class="mb-8 md:mb-12 bg-[#141414]/60 rounded-xl p-6 shadow-lg">
        <h1 class="text-4xl md:text-6xl font-bold text-[#F8F8F8]">Experience</h1>
        <p class="text-lg text-[#EEDC82] mt-4">Places I've worked and grown</p>
      </div>
      ...
      <div class="space-y-24" ref="experienceContainer">
        ...
        >
          <!-- Background number -->
          <div
              class="absolute -top-16 opacity-10 text-8xl md:text-9xl font-bold text-[#EEDC82]"
          ...
          <div class="relative z-10">
            <!-- Header -->
            <div class="flex bg-[#141414]/60 rounded-xl p-4 sm:p-6 items-center justify-between mb-6 shadow-lg">
              <div style="width: 50%">
                <h2 class="text-3xl font-bold text-[#F8F8F8]">{{ experience.company }}</h2>
                <h3 class="text-[#EEDC82] mt-1 text-sm">{{ $formatDate(experience.start,'{y}-{m}-{d}') }} - {{ experience.end ? $formatDate(experience.end,'{y}-{m}-{d}') : '至今' }}</h3>
                <p class="text-[#EEDC82] leading-relaxed mb-6 text-sm md:text-base text-left" v-html="formattedContent(experience.description)"></p>
              </div>
              <span class="text-3xl text-[#F8F8F8]" style="width: 50%">{{ experience.role }}</span>
            </div>
            <!-- Main content -->
            <div
                ...
                <div class="grid grid-cols-2 gap-3 w-full [&>*:only-child]:col-span-2" v-if="experience.project">
                  ...
                  <template v-for="(project, idx) in experience.project.slice(0, 3)" :key="project.id">
                    <div
                        ...
                        class="group relative bg-[#0a0a0a]/70 rounded-lg overflow-hidden cursor-pointer w-full shadow-lg"
                    >
                      <div class="aspect-square">
                        ...
                        <div class="absolute inset-0 p-3 flex flex-col justify-end">
                          <span class="text-sm font-medium text-[#F8F8F8]">{{ project.title }}</span>
                          <span class="text-xs text-[#EEDC82]">{{ $formatDate(project.start,'{y}-{m}-{d}') }} - {{ project.end ? $formatDate(project.end,'{y}-{m}-{d}') : '至今' }}</span>
                        </div>
                      </div>
                    </div>
                  </template>
                  ...
                  <div
                      ...
                      class="group relative bg-[#0a0a0a]/70 rounded-lg overflow-hidden cursor-pointer
                           border border-[#EEDC82]/30 hover:border-[#EEDC82]/50 transition-colors w-full shadow-lg"
                  >
                    <div class="aspect-square flex items-center justify-center" @click="toRoute('/about/project?experience_id='+experience.id)">
                      <div class="text-center">
                        <div class="mb-1">
                          <PlusCircleIcon class="w-6 h-6 mx-auto text-[#EEDC82] group-hover:text-[#F8F8F8] transition-colors" />
                        </div>
                        <span class="text-xs font-medium text-[#EEDC82] group-hover:text-[#F8F8F8] transition-colors">
      ...
      <div v-if="loading" class="text-center py-8">
        <div class="inline-block animate-spin rounded-full h-8 w-8 border-t-2 border-[#EEDC82]"></div>
      </div>
    </div>
  </div>
</template>
