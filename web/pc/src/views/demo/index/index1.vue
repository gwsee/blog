...
const todos = ref([
  ...
  },
  {
    id: 2,
    theme: '学习Vue 3新特性',
    content: '理解Composition API和响应式系统',
    status: 'pending',
    type: 2,
    fromDate: new Date(),
...
const resetTodoForm = () => {
  ...
  currentTodo.type = 1
  currentTodo.times = 1
  currentTodo.fromDate = new Date()
  currentTodo.toDate = new Date(new Date().setDate(new Date().getDate() + 7))
}
...
const openTodoDialog = (mode, todo = null) => {
  ...
  if (todo) {
    ...
    currentTodo.times = todo.times || 1
    currentTodo.fromDate = todo.fromDate || new Date()
    currentTodo.toDate = todo.toDate || new Date(new Date().setDate(new Date().getDate() + 7))
  }
...
const saveTodo = () => {
  ...
  if (todoDialogMode.value === 'add') {
    todos.value.push({
      id: Date.now(),
      theme: currentTodo.theme,
      content: currentTodo.content,
      status: 'pending',
      type: currentTodo.type,
      times: currentTodo.type === 1 ? currentTodo.times : undefined,
      fromDate: currentTodo.type === 2 || currentTodo.type === 3 ? currentTodo.fromDate : undefined,
      toDate: currentTodo.type === 2 ? currentTodo.toDate : undefined,
      createdAt: new Date()
    })
  } else if (todoDialogMode.value === 'edit') {
    const index = todos.value.findIndex(t => t.id === currentTodo.id)
    if (index !== -1) {
      todos.value[index] = {
        ...todos.value[index],
        theme: currentTodo.theme,
...
// 右侧备忘录
const memos = ref([
  { id: 1, name: 'Vue 3学习', content: 'Vue 3 Composition API的基本用法', familiarity: 3 },
  { id: 2, name: 'Tailwind技巧', content: '记住在Tailwind中使用flex-wrap处理响应式布局', familiarity: 4 },
  { id: 3, name: '园林设计', content: '学习中国古典园林设计的核心理念：借景、框景、对景', familiarity: 2 }
])
const newMemo = ref('')
...
const familiarityDialogVisible = ref(false)
...
const openMemoDialog = (mode, memo = null) => {
  ...
  if (memo) {
    ...
    currentMemo.familiarity = memo.familiarity
  }
...
const saveMemo = () => {
  ...
  if (memoDialogMode.value === 'add') {
    memos.value.push({
      ...
      familiarity: currentMemo.familiarity
    })
...
const openFamiliarityDialog = (memoId) => {
  ...
  familiarityDialogVisible.value = true
}

const closeFamiliarityDialog = () => {
  familiarityDialogVisible.value = false
...
const updateMemoFamiliarity = (level) => {
  ...
  if (index !== -1) {
    memos.value[index].familiarity = level
  }
...
<template>
  <div class="h-full w-full overflow-hidden bg-transparent text-gray-800 relative md:my-6">
              ...
              <li v-for="memo in memos" :key="memo.id"
                ...
                <div class="overflow-hidden pr-2">
                  <div @click="showMemoDetails(memo)" class="cursor-pointer">
                    ...
                    <p class="text-sm text-gray-600 truncate">{{ memo.content }}</p>
                  </div>
                </div>
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <div class="flex items-center space-x-2">
              <button v-for="n in 5" :key="n" @click="currentMemo.familiarity = n"
                      :class="[
                  'w-8 h-8 rounded-full text-sm flex items-center justify-center transition-colors',
                  currentMemo.familiarity >= n ? familiarityColor(n) : 'bg-gray-100'
                ]">
                {{ n }}
              </button>
            </div>
      ...
      <div class="bg-white/95 rounded-lg p-6 w-full max-w-md shadow-xl" @click.stop>
        ...
        <div v-if="selectedMemo" class="space-y-4">
          ...
          <div>
            ...
            <p class="mt-1 whitespace-pre-line">{{ selectedMemo.content }}</p>
          </div>
          ...
          <div>
            ...
            <div class="mt-1 flex items-center space-x-1">
              <div v-for="n in 5" :key="n"
                  ...
                  selectedMemo.familiarity >= n ? familiarityColor(n) : 'bg-gray-100/50'
                ]">
                {{ n }}
              </div>
            </div>
    ...
    <!-- 熟悉度选择对话框 -->
    <div v-if="familiarityDialogVisible" class="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 flex items-center justify-center" @click="closeFamiliarityDialog">
      ...
      <div class="bg-white/95 rounded-lg p-6 w-full max-w-md shadow-xl" @click.stop>
        ...
        <div class="space-y-4">
          ...
          <div v-if="currentTodo.type === 1">
            ...
            <input
                v-model.number="currentTodo.times"
                type="number"
                min="1"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-green-200 focus:border-green-300"
            />
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <button
                  @click="currentTodo.status = 'pending'"
                  :class="[
                  'px-3 py-2 border rounded-md text-sm',
                  currentTodo.status === 'pending' ? 'bg-green-100 border-green-300' : 'border-gray-300 hover:bg-gray-50'
                ]"
              >
                待办
              </button>
              <button
                  @click="currentTodo.status = 'completed'"
                  :class="[
                  'px-3 py-2 border rounded-md text-sm',
                  currentTodo.status === 'completed' ? 'bg-green-100 border-green-300' : 'border-gray-300 hover:bg-gray-50'
                ]"
              >
                已做
              </button>
    ...
    <div v-if="todoDetailsVisible" class="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 flex items-center justify-center" @click="closeTodoDetails">
      <div class="bg-white/95 rounded-lg p-6 w-full max-w-md shadow-xl" @click.stop>
        ...
        <div v-if="selectedTodo" class="space-y-4">
          ...
          <div>
            <h4 class="text-sm font-medium text-gray-500">内容</h4>
            <p class="mt-1 whitespace-pre-line">{{ selectedTodo.content }}</p>
          </div>
          ...
          <div v-if="selectedTodo.type === 1">
            ...
            <p class="mt-1">{{ selectedTodo.times || 1 }}</p>
          </div>

          <div v-if="selectedTodo.type === 2 || selectedTodo.type === 3">
            <h4 class="text-sm font-medium text-gray-500">开始日期</h4>
            <p class="mt-1">{{ selectedTodo.fromDate ? new Date(selectedTodo.fromDate).toLocaleDateString() : '未设置' }}</p>
          </div>

          <div v-if="selectedTodo.type === 2">
            <h4 class="text-sm font-medium text-gray-500">结束日期</h4>
            <p class="mt-1">{{ selectedTodo.toDate ? new Date(selectedTodo.toDate).toLocaleDateString() : '未设置' }}</p>
          </div>

          <div>
            <h4 class="text-sm font-medium text-gray-500">状态</h4>
            <p class="mt-1">{{ selectedTodo.status === 'pending' ? '待办' : '已完成' }}</p>
          </div>

          <div>
            <h4 class="text-sm font-medium text-gray-500">创建时间</h4>
            <p class="mt-1">{{ selectedTodo.createdAt ? new Date(selectedTodo.createdAt).toLocaleString() : '未知' }}</p>
          </div>
        </div>
      </div>
    </div>
