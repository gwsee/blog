<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- 标题区域 -->
      <div class="mb-8">
        <h1 class="text-4xl font-bold text-gray-900 mb-4 font-sans">{{ travel.title }}</h1>
      ...
      <div class="relative mb-8 rounded-2xl overflow-hidden shadow-lg">
        <a-carousel class="aspect-[16/9] h-[70vh]" :autoplay="false" ref="carouselRef">
          <div v-for="(image, index) in travel.images" :key="index"
            ...
            <div class="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div class="absolute bottom-4 left-4 text-white">
                <p class="text-lg font-medium">{{ travel.imageDescriptions[index] }}</p>
              </div>
            </div>
          </div>
        </a-carousel>
        ...
        <button @click="prevSlide" class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/20 hover:bg-black/40 text-white p-3 rounded-full transition-colors duration-300 backdrop-blur-sm">
          <left-outlined class="text-xl" />
        </button>
        <button @click="nextSlide" class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/20 hover:bg-black/40 text-white p-3 rounded-full transition-colors duration-300 backdrop-blur-sm">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          <!-- 简介 -->
          <a-card class="shadow-sm rounded-2xl">
            <template #title>
              <div class="flex items-center space-x-2">
                <info-circle-outlined />
                <span class="font-medium">旅行简介</span>
              </div>
            </template>
          ...
          <!-- 视频展示 -->
          <a-card class="shadow-sm rounded-2xl overflow-hidden">
            <template #title>
              <div class="flex items-center space-x-2">
                <video-camera-outlined />
                <span class="font-medium">精彩视频</span>
              </div>
            </template>
        ...
        <div class="space-y-6">
          ...
          <a-card class="shadow-sm rounded-2xl">
            <template #title>
              <div class="flex items-center space-x-2">
                <user-outlined />
                <span class="font-medium">关于作者</span>
              </div>
            </template>
...
const travel = ref({
  ...
  images: [
    'https://gwsee-blog.oss-cn-chengdu.aliyuncs.com/image/png/20241211/10002.png?x-oss-credential=LTAI5t5j88DF7KKibWGDMesb%2F20241211%2Fcn-chengdu%2Foss%2Faliyun_v4_request&x-oss-date=20241211T031518Z&x-oss-expires=86400&x-oss-signature=a5c076c4d7d411bc889d0d5785b2cd2bbe6727dec8101da8a2d6a08c28768e6f&x-oss-signature-version=OSS4-HMAC-SHA256',
...
onMounted(() => {
  // Vue 3 中不再使用 $on，而是使用 watch 来监听变化
