...
const fetchTodoList = ()=>{
  todoList({status:0,page:{pageSize:15,pageNum:1}}).then(res=>{
    if(res && res.code === 200){
...
<template>
  <div class="min-h-full w-full overflow-auto bg-transparent text-gray-800 relative md:my-6">
    <!-- 移动端菜单按钮 -->
    ...
    <div class="max-w-full mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 min-h-screen">
      ...
      <!-- 移动端布局 - 垂直堆叠 -->
      <template v-else>
        <div class="flex flex-col min-h-screen pb-16 overflow-auto">
          <div class="pb-20">
            <!-- 顶部区域 -->
            <div class="flex-grow">
              <!-- 公告区域 - 即使隐藏也保留空间 -->
              ...
              <!-- 待办事项 - 隐藏时保留占位符 -->
              <div v-if="showLeftSection" class="bg-white/30 backdrop-blur-sm p-4 rounded-lg shadow-sm border border-gray-200/50 my-4 overflow-visible">
                <div class="flex justify-between items-center mb-4">
                  <h2 class="text-lg font-semibold text-gray-700 flex items-center">
                    <UnorderedListOutlined class="mr-2" />待办事项
                  </h2>
              ...
              <!-- 备忘录 - 隐藏时保留占位符 -->
              <div v-if="showRightSection" class="bg-white/30 backdrop-blur-sm p-4 rounded-lg shadow-sm border border-gray-200/50 my-4 overflow-visible">
                <div class="flex justify-between items-center mb-4">
                  <h2 class="text-lg font-semibold text-gray-700 flex items-center">
                    <FileTextOutlined class="mr-2" />备忘录
                  </h2>
